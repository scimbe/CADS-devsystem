...
| **Stand der Technik** (state of the art) | Current, non-deprecated dependencies and idioms at time of delivery | **Partially checked, two of three real gaps closed as of 2026-08-09.** `.github/dependabot.yml` (`CADS-devsystem@9c97211`/`8949cbf`) runs a real weekly `cargo` freshness check against both crates plus GitHub Actions versions. Three real Dependabot PRs opened from it (`rand` 0.8.7→0.10.2 in both crates, `ed25519-dalek` 2.2.0→3.0.0 in `web`) all genuinely failed CI as of 2026-08-07, one real correlated root cause across all three (`rand` 0.10's `OsRng` moved out of `rand::rngs`; `ed25519-dalek` 3.0.0's own `rand_core` bump then isn't satisfied by the `OsRng` the currently-pinned `rand` provides). **The `rand` half fixed for real, both crates**: `CADS-devsystem@573846e` bumps `rand` to 0.10.2 in `pipeline`/`web` directly, adds `rand_core = "0.6"` as an explicit direct dependency (`ed25519-dalek` 2.x's own real, pinned RNG-trait bound), and switches the one real `rand::rngs::OsRng` call site (`quick_submit_offer`'s ephemeral signing key) to `rand_core::OsRng` — hermetically verified, both crates build clean under `-D warnings`. Both Dependabot `rand` PRs (#9, #10) auto-closed as superseded. Live-confirmed current: `pipeline/Cargo.lock` and `web/Cargo.lock` both genuinely pin `rand 0.10.2` today. **What's still genuinely open**: `ed25519-dalek` 3.0.0 ([PR #11](https://github.com/scimbe/CADS-devsystem/pull/11)) still fails CI on its own, unrelated to the `rand` fix — its own major-version `rand_core` bound is a real, separate compatibility question, reviewing/merging it is the operator's own call (not scimbe-authored, out of scope here) |
...
| **Fachgerecht / Fachmännisch** (professionally correct) | Passes the same review bar a competent human reviewer would apply | **Corrected, 2026-08-10** -- this row went stale without being updated when the work below closed. Marking a requirement `verified` is now a real, hard `409` (not advisory) unless a qualifying `devsystem.review` iteration actually addresses it, and that review itself must clear real mechanical bars (length, distinct words, and -- as of `CADS-devsystem@6836977` -- non-generic content) to qualify at all; see the ranked list below (items closing issues #39/#42/§8's "generic-but-varied" gap) for the real evidence. **Still genuinely partial, scoped precisely**: this only gates the `verified` transition on a *requirement*, not every iteration in general -- an iteration can still be submitted `succeeded: true` on a stage with no review role declared at all, or on one that never touches a requirement's `verified` flag, with zero review gate in its way. That broader "no change ships without review, full stop" claim remains open |
...
**Corrected, 2026-08-10 -- this paragraph's own "remains the real, open, harder half" framing went
stale without being updated here when the work closed.** The structural precondition described
above IS now real, precisely scoped: `qualifying_review_evidence` (`pipeline/src/runner.rs`) hard-
blocks (`409`) marking a *requirement* `verified` without a real, substantive `devsystem.review`
iteration addressing it -- not advisory, not beatable by a rubber-stamp (length/distinct-word/
generic-content bars all real and mutation-tested), live-verified against the actual deployment.
`final_iteration_blocked_by_unanswered_decision` further closes the specific deadlock the original
evaluator report on issue #39 hit (a run reaching its ceiling with an operator's real answer having
nowhere to land). **What's still genuinely open, precisely**: this gate keys on a requirement's own
`verified` transition, not on `succeeded: true` in general -- a stage that never claims a
requirement via `requirement_indices` can still submit `succeeded: true` iterations with zero review
in its way. `no_review_for_succeeded_work` (still advisory, `pipeline/src/preflight.rs`) is the only
signal covering that broader case today. Turning *that* into a hard block too -- gating every
`succeeded: true` submission, not just ones that touch a requirement -- is the real, still-open,
harder half now, and was one of the three genuinely undecided options the operator was asked about
on `#382` (still no reply) -- not something to guess at and ship unilaterally.
...
**Goal-driven-loop firing, 2026-08-10 (qqqq) -- §5's own "Stand der Technik" row was stale; corrected
against real, live evidence.** State check: no new scimbe reply on any of `#382`'s open checkpoints;
no new open issues/PRs. Systematic sweeps for a fresh confirm()-gap or an `owner_authorized` gap (the
two patterns that found the most real bugs this session) both came back clean this time -- every
mutating GUI button already confirms where it should, every mutating handler already checks
ownership. Read §5's own quality-bar table directly instead of inventing busywork, and found a real,
concrete staleness: it still says "three real, currently-open PRs exist right now" for the
dependency-freshness gate, checked live 2026-08-07.

Checked live, not assumed: two of those three (`rand` 0.8.7→0.10.2 in both `pipeline` and `web`,
Dependabot PRs #9/#10) closed 2026-08-09, superseded by a real fix pushed directly to `main`
(`CADS-devsystem@573846e`) -- bumps `rand` to 0.10.2 in both crates, adds `rand_core = "0.6"` as an
explicit direct dependency (`ed25519-dalek` 2.x's own real, pinned RNG-trait bound, which the new
`rand` no longer satisfies through its own re-export), and switches the one real
`rand::rngs::OsRng` call site (`quick_submit_offer`'s ephemeral signing key) to `rand_core::OsRng`.
Live-confirmed current: both `Cargo.lock` files genuinely pin `rand 0.10.2` today. The third,
`ed25519-dalek` 3.0.0 ([PR #11](https://github.com/scimbe/CADS-devsystem/pull/11)), is real, separate,
and still genuinely fails CI on its own -- re-checked its live CI status rather than assuming it
